...
즉, A는 자신이 사용할 객체에 대해 추상적으로만 알고, 그저 `i`에 할당된 인스턴스에  `method` 라는 메서드가 존재한다는 것만 알고 있는 상
...
위와 같이 코드를 작성했을 때, A는 저 인터페이스 I의 method에 대해 알지 못해. 그냥 선언을 했을 뿐이지. 그렇

다면 구현체인 B와 C중에서 어떤 객체에 의존성을 가져야할지만 결정하면 되는거야. 
이러한 의존성을 결정하는 것을 스프링을 통해 할 수 있어. 우린 간단하게 설정값만 변경함으로써 A에 B의 의존성을 주입할 수도, C의 의존성을 주입할 수도 있는거지. 
이렇게 되면 코드의 수정 없이 새로운 객체를 생성하고, 설정값만 변경함으로써 문제를 해결할 수 있는거야.

이렇게 ==객체의 의존성을 개발자가 아닌 스프핑이 주입하게 하는것을 Inversion Of Control 제어의 역전== 이라고 부르고 의존성을 주입하는 행위 자체를 ==Dependency Injection, 의존성 주입==이라고 불러 
